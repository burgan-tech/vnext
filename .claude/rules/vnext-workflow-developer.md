...
- **Response-shape version**: `StateFunctionCache.ResponseShapeVersion` (currently `v8`) is folded into both the ETag material and the cache key. Bump it in the same commit as any change to what the state body carries — otherwise a client polling a parked instance keeps getting 304 and never sees the new shape.
- **`incident` block**: always present — `{ hasActiveIncident, active: { id, errorCode, message, state, transition, task, errorLayer, statusCode, boundaryAction, traceId, createdAtUtc } | null, historyHref }`. Lifted from the leaf when an active subflow reports one; `historyHref` → `GET …/instances/{instance}/incidents` (paged, same `queryRoles` gate). No stack trace. `HasActiveIncident` is a fingerprint member so raise/resolve without a state change moves the ETag; resolve-A-then-raise-B in one parked state is a documented stale-`active` gap.
