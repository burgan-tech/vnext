...
├─ BackgroundJob.Arm/{type}/{key}         async accept — the Dapr scheduler arm round-trip
...
│     ├─ Instance.Query.Prepare          query/include construction
│     └─ Instance.Query.Execute          compile → command(s) → materialize
...
| `Step.{Name}` | `BBT.Workflow.Pipeline` | `vnext.step.order`, `vnext.step.outcome` (continue / stop / skipTo:{order}), `vnext.transition.key`, `span.category=business` | One per pipeline step **that did work** (`PipelineStepActivityHelper.StartStepActivity`). The transition tag identifies the owning hop. A step reporting `StepOutcome.ContinueNoWork()` has its span dropped from export (Recorded cleared), so non-applicable steps leave no trace. Trailing `Step` trimmed from the class name for display. |
| `Transition.LoadContext` | `BBT.Workflow.Pipeline` | `span.category=business`, `vnext.transition.key` | Wraps `TransitionContextFactory`; child `Cache.Get`/`Instance.Load` spans attach automatically. `CreateFromPreloaded` is deliberately left unwrapped (no fresh load happened). |
| `Instance.Load` | `BBT.Workflow.Pipeline` | `span.category=business` | Wraps `instanceRepository.GetActiveAsync`. |
| `Instance.Query.Prepare` | `BBT.Workflow.Pipeline` | `span.category=business` | Wraps detailed-query/include construction (`WithDetailsAsync`) before execution. Child of `Instance.Load`; immediately followed by `Instance.Query.Execute`. Uses implicit parenting so baggage flows through. |
| `Instance.Query.Execute` | `BBT.Workflow.Pipeline` | `span.category=business` | Wraps EF query compilation/setup, lazy connection acquisition, all `Db.SELECT` children and materialization. Emitted by the detailed identifier/key finders and the focused subflow-start loader. This is the span that closes the residual gap between `Instance.Query.Prepare` and the first database command. |
| `Transition.Validate` | `BBT.Workflow.Pipeline` | `span.category=business`, `vnext.transition.key`, `ActivityStatusCode.Error` + message on failure | Wraps schema validation + `TransitionExecutionPolicy`. |
| `Schema.Validate` | `BBT.Workflow.Pipeline` | `span.category=business`, `vnext.transition.key`, `ActivityStatusCode.Error` + message on failure | Wraps the synchronous JSON-schema evaluation after the schema definition has been resolved from cache. Separates schema parsing/evaluation CPU from the preceding cache lookup. |
| `Instance.Persist` | `BBT.Workflow.Pipeline` | `span.category=business` | Wraps initial instance insertion, including EF change tracking / SaveChanges preparation and its `Db.INSERT` child. |
...
| `Transition.Continuation/{mode}` | `BBT.Workflow.Pipeline` | `vnext.transition.key`, `vnext.continuation.mode`, `vnext.continuation.has_next`, error status on failure | What happens between one hop finishing and the next starting. Current execution emits `Inline`, which hands the next context to the loop; `Enqueue` is a legacy enum/implementation and is not registered. |
...
| `Script.Execute` | `BBT.Workflow.Scripting` | `vnext.script.kind` ∈ `lockKey` \| `subflowInputMapping` \| `subflowOutputMapping` \| `functionOutput` | Wraps compile plus invocation at a call site with no existing delimiting span. Task input/output mappings are deliberately **not** wrapped separately — `Task.PrepareInput`/`Task.ProcessOutput` already delimit them. For subflow input mappings, `Script.Compile/{identity}` and `Script.Invoke` children split cold compilation from execution. The `subflowOutputMapping` span also covers the downstream `AppendAsync`/`UpdateAsync` persistence; Task 9's `Instance.AppendData` child now makes that timing decomposable. The `functionOutput` kind covers the function's `IOutputHandler` script invocation in `FunctionAppService`, nested under `Function.BuildResponse`. **Controller ruling**: `Script.Execute` deliberately carries no `vnext.script.key` — the `vnext.script.kind` tag plus the parent span's context (the enclosing step/subflow/function span) are enough to identify which script ran; `Script.Compile` carries identity in its NAME on every compile, and its `vnext.script.key` tag remains cold-path-only (see above). |
| `Script.Invoke` | `BBT.Workflow.Scripting` | `span.category=business` | Child of a subflow-input `Script.Execute`; covers only the already-compiled mapping delegate call. This prevents cold traces from showing the post-compile execution time as an unexplained blank region. |
| `SubFlow.Start/{domain}/{flow}` | `BBT.Workflow.SubFlow` | `vnext.subflow.operation=start`, `vnext.subflow.type`, parent/subflow/root instance identifiers | Covers the complete subflow-start operation, including the optional input mapping, request construction and local/remote start call. The input mapping's `Script.Execute` is a child, so time between compile, invocation and dispatch stays contained rather than rendering as sibling gaps. |
...
| `PostCommit.Coordinate` | `BBT.Workflow.Pipeline` | `span.category=business`, `vnext.transition.key`, error status on failure | Wraps creation of the fresh post-commit scope and `PostCommitTransitionCoordinator`. The scope reuses the committed context's definition (`resolvedWorkflow`), so there is no `Cache.Get/sys-flows:…` child any more; the `PostCommit.*` job spans are its children, so the interval immediately after `Uow.Commit` is fully owned. |
| `PostCommit.Settle` / `PostCommit.Fault` | `BBT.Workflow.Pipeline` | `span.category=business`, `vnext.transition.key`, `vnext.instance.id`, error status when the mutation fails | The fresh-parent mutation that follows coordination (`TransitionRunner.MutateParentAsync` → `PostCommitParentMutationService.SettleAsync` / `FaultAsync`): a `HandoffToChild` (or a `ContinueParent` with nothing left to continue) settles the parent from an authoritative reload; a post-commit failure carrying a `PostCommitFaultRequest` faults it. Its children are the phase that used to land on the transaction as unrelated siblings after `PostCommit.Coordinate`: `Lock.Acquire/{lockKey}` (`kind=status`), the settlement reload's `Db.SELECT`s (`FindForPostCommitSettlementAsync`, split query: instance + open correlations always, the latest data row only for a sync caller or a fault — see the decision matrix on `PostCommitParentMutationService.NeedsLatestDataForSettle`), `Transition.Settle`, `Uow.Commit`, `Lock.Release/{lockKey}`. The scope reuses the snapshot's definition — no `Cache.Get`. The job handler's final `Db.UPDATE` (`InstanceJobs` mark-as-processed, `TransitionJobHandler` `finally`) is **not** inside it: that write belongs to the job, not to the settlement. |
...
`Instance.Query.Prepare` wraps query/include construction (`WithDetailsAsync()`).
`Instance.Query.Execute` begins immediately afterwards and wraps the complete materialization call,
including EF query compilation, lazy connection acquisition, every `Db.SELECT`, and result shaping.

**How to read a measurement once this span is live:**

- **`Instance.Query.Prepare` dominates** → query/include construction is expensive.
- **The leading self-time inside `Instance.Query.Execute` dominates** → EF compilation/setup or
  lazy connection acquisition is expensive; inspect compiled-query usage and Npgsql pool metrics.
- **Its `Db.SELECT` children dominate** → the cost is in PostgreSQL/network execution.
- **The trailing self-time inside `Instance.Query.Execute` dominates** → result materialization and
  EF fix-up/change tracking are expensive.
...
- **`Instance.Read/state` and `Instance.Read/data`, `Instance.Read.BuildGate`, and the two
  zero-document transaction tags** are not shipped yet. They were sequenced behind the
  effective-status-fingerprint change, which has since landed on master (`#983`), so the sequencing
  condition is satisfied and they are simply next.
...
- **Six contested families** (`Auth.FilterTransitions`, `Auth.PreviousUserLookup`, the function
  path's `Schema.Validate`, `View.Resolve`, `Inbox.Forward`, `Event.Intake`) ship only with a named
  question each answers, recorded in its row here.
