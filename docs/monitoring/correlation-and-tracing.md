...
| Activation episode (`EpisodeStartedAt` / `EpisodeTrigger` / `EpisodeTransitionKey`) | **Span duration**: when the trigger that set the instance in motion was accepted, so the hop that brings it to rest can emit the backdated `Instance.Activation/{key}` span | The same carriers as the lane anchor, beside it — three nullable fields always copied together. A timestamp, not an anchor, so it cannot graft spans onto a foreign trace. See [Trace Lanes § Activation episode](../runtime/trace-lanes.md#activation-episode) |
...
episode's three fields (stamped `??=`-style — a preset value is never overwritten) — see
