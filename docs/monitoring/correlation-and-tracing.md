...
| `X-Request-Id` / `vnext.request.id` | **Per client HTTP request.** Read/generated by Aether's `AetherCorrelationIdMiddleware`; joins every log line back to the request that caused it | Request header; job payload `Headers`; event contract `RequestId`; `TaskTraceContext.RequestId`; log enricher `RequestHeader.x_request_id` |
