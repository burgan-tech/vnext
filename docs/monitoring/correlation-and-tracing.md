...
time, and restored by `EventTraceScope` in the Inbox handlers. Events that open a top-level operation
on the consuming side (the transition continuation, the subflow terminal events) additionally
implement `ILaneAwareDistributedEvent` and carry the lane anchor — see
