    ...
    : ApplicationService(serviceProvider), IInstanceQueryAppService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // The ETag / 304 decision is made only AFTER authorization and the full representation
        // build: the state representation depends on request inputs (input.Role, headers, query
        // params, resolved culture, dynamic grants) as well as server state, and access can be
        // revoked server-side without any request input changing. A cheap pre-auth change-token
        // fast path was tried and reverted (review B3/C1) because it both masked authorization
        // denials with a 304 and produced stale 304s when a representation-affecting request
        // input changed without altering the token. The per-poll cost is already reduced by
        // latest-only instance loading (WorkflowExecution:LatestOnlyInstanceLoading); a correct
        // cheap-304 needs a dedicated design (bind every representation input into the ETag AND
        // run authorization before the 304) with integration tests.
