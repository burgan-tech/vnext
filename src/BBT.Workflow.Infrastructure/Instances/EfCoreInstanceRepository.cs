        ...
        IInstanceRepository
{
    ...
    /// <inheritdoc />
    public async Task<InstanceStateSignal?> GetStateSignalAsync(string identifier,
        CancellationToken cancellationToken = default)
    {
        var dbSet = await GetDbSetAsync();
        ...
        if (Guid.TryParse(identifier, out var instanceId))
        {
            var byId = await QueryStateSignalAsync(query.Where(i => i.Id == instanceId), cancellationToken);
            if (byId is not null)
            {
                return byId;
            }
        ...
        // Same fallback semantics as FindByIdentifierAsync: most recent row for the key.
        return await QueryStateSignalAsync(
            query.Where(i => i.Key == identifier).OrderByDescending(i => i.CreatedAt),
            cancellationToken);
    }

    /// <summary>
    /// Projects the change-detection signal without materializing the aggregate. The latest
    /// data ETag lookup is served by the covering unique index
    /// <c>UX_InstancesData_Instance_IsLatest</c> (INCLUDE ETag), so the whole signal costs a
    /// single small query.
    /// </summary>
    private static async Task<InstanceStateSignal?> QueryStateSignalAsync(
        ...
        CancellationToken cancellationToken)
    {
        var row = await query
            .Select(i => new
            {
                i.Id,
                i.CurrentState,
                i.CurrentStateSubType,
                i.Status,
                i.ModifiedAt,
                LatestDataEtag = i.DataList
                    .Where(d => d.IsLatest)
                    .Select(d => d.ETag)
                    .FirstOrDefault(),
                Correlations = i.ChildCorrelations
                    .Where(c => !c.IsCompleted)
                    .Select(c => new { c.Id, c.SubFlowCurrentState, c.SubFlowStateChangedAt })
                    .ToList()
            })
            .FirstOrDefaultAsync(cancellationToken);
        ...
        return new InstanceStateSignal(
            row.Id,
            row.CurrentState,
            row.CurrentStateSubType,
            row.Status,
            row.ModifiedAt,
            row.LatestDataEtag,
            row.Correlations
                .Select(c => new InstanceCorrelationSignal(c.Id, c.SubFlowCurrentState, c.SubFlowStateChangedAt))
                .ToList());
    }
