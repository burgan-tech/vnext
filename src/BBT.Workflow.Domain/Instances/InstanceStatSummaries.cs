...
public sealed record TaskExecutionStat(string TaskKey, int ExecutionCount, double AvgDurationMs, int SuccessCount, int FailureCount);

/// <summary>Per-transition execution aggregation (keyed by transition + from/to state pair) across a schema.</summary>
public sealed record TransitionExecutionStat(string TransitionKey, string? FromState, string? ToState, int Count, double AvgDurationMs, int CompletedCount, int ManualCount, int AutomaticCount, int ScheduledCount, int EventCount);
