...
{
    /// <summary>
    /// Validates only the request payload against the transition input schema.
    /// Use this at the admission boundary so invalid requests fail before Busy/job side effects.
    /// </summary>
    Task<Result> ValidateInputSchemaAsync(
        TransitionExecutionContext context,
        CancellationToken cancellationToken = default);

    /// <summary>
    /// Validates only the state/actor transition policy against the current instance snapshot.
    /// This can be repeated after an authoritative reload without repeating schema/cache work.
    /// </summary>
    ...
    /// 1. Validates input data against the transition's schema if present.
