...
public sealed class ErrorBoundaryValidator
{
    ...
    /// <param name="validStates">Set of valid state keys for transition validation.</param>
        ...
        HashSet<string> validStates)
    {
        ...
        if (errorBoundary.OnError.Count > 0)
        {
            for (var i = 0; i < errorBoundary.OnError.Count; i++)
            {
                ...
                results.AddRange(ValidateErrorHandlerRule(rule, ruleContext, validStates));
            }
        ...
        HashSet<string> validStates)
    {
        ...
        {
            if (!validStates.Contains(rule.Transition))
            {
                results.Add(new ValidationResult(
                    $"Transition '{rule.Transition}' does not match any valid state.",
                    [$"{context}.{nameof(ErrorHandlerRule.Transition)}"]));
            }
        }

        // Validate NotificationConfig when Action is Notify
        if (rule.Action == ErrorAction.Notify && rule.NotificationConfig == null)
        {
            results.Add(new ValidationResult(
                "NotificationConfig is recommended when Action is Notify.",
                [$"{context}.{nameof(ErrorHandlerRule.NotificationConfig)}"]));
        }
